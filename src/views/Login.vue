<template>
  <div class="min-h-full flex items-center justify-center py-4 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      ...
      <form class="mt-4 space-y-3 border p-2 border-gray-400 rounded-2xl" method="POST" @submit.prevent="handleSubmitForm">
        <Alert />
        ...
        <div>
          <button type="submit" class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" :disabled="authenticationAnimation.type === 'sign-in'">
            <span class="absolute left-0 inset-y-0 flex items-center pl-3">
              <LockClosedIcon class="h-5 w-5 text-indigo-500 group-hover:text-indigo-400" aria-hidden="true" />
            </span>
            <CirclesToRhombusesSpinner
              v-if="authenticationAnimation.type === 'sign-in'"
      ...
      <div>
        ...
        <button class="my-2 group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" :disabled="authenticationAnimation.type === 'guest'">
          <span class="absolute left-0 inset-y-0 flex items-center pl-3">
            <FontAwesomeIcon :icon="['fas', 'user-astronaut']" class="h-5 w-5 text-indigo-500 group-hover:text-indigo-400" aria-hidden="true" />
          </span>
          <CirclesToRhombusesSpinner
            v-if="authenticationAnimation.type === 'guest'"
            :animation-duration="1200"
            :circles-num="3"
            :circle-size="15"
            color="#fff"
